...
if __name__ == '__main__':
    import time
    # preprare data, the data should be in list format
    data = [i for i in range(100)]
    
    # define a function to process data
    def do(process_id, idx, data, writer):
        print(data)
        # you could not care what the process_id and idx are, just process the data and write the result into the writer
        writer.write(f"Process{process_id} processed data {data}\n")
        time.sleep(0.5)
    
    # create a MultipleProcessRunnerSimplifier object
    # you could pass n_process to specify the number of process
    # save_path is a file path to save the results
    mpr = MultipleProcessRunnerSimplifier(data, do, save_path="mpr_result.txt", n_process=4, verbose=True, total_only=False, log_step=10)
    mpr.run()
...
import os
from tqdm import tqdm
def read_amino_acid_sequence_from_pdb(file_path):
    with open(file_path, 'r') as file:
        sequences = {}
        current_residue_num = None
        for line in file:
            if line.startswith("ATOM") and line[13:15] == "CA":  # Check for alpha carbon to ensure one entry per residue
                residue_name = line[17:20].strip()
                chain_id = line[21].strip()
                residue_num = line[22:26].strip()
                
                # Initialize the sequence for the chain if not already done
                if chain_id not in sequences:
                    sequences[chain_id] = []

                # To avoid adding the same residue in case of multiple atoms per residue
                if residue_num != current_residue_num:
                    sequences[chain_id].append(residue_name)
                    current_residue_num = residue_num

        # Convert the three-letter codes to single-letter codes, assuming you have a conversion dictionary
        three_to_one = {
        ...
        # Convert sequences from three-letter to single-letter code
        for chain in sequences:
            sequences[chain] = ''.join([three_to_one[res] for res in sequences[chain] if res in three_to_one])

    return sequences

pdb_dir = "/zhouxibin/workspaces/ESM-Ezy/data/af_uniref50/overlap"
output_fasta_path = "/zhouxibin/workspaces/ESM-Ezy/data/af_uniref50/overlap.fasta"
file_paths = []
for path in tqdm(os.listdir(pdb_dir)):
    file_paths.append(os.path.join(pdb_dir, path))
def do(process_id, idx, path, writer):
    uniprot_id = path.strip().split("/")[-1].split(".")[0]
    name = ">"+uniprot_id
    sequence = read_amino_acid_sequence_from_pdb(path)
    if "A" not in sequence or len(sequence["A"]) == 0:
        return
    writer.write(name+'\n'+sequence["A"]+'\n')

mpr = MultipleProcessRunnerSimplifier(file_paths, do, output_fasta_path, n_process=16)
mpr.run()
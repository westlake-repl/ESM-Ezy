...
class Downloader(MultipleProcessRunner):
	...
	def _target(self, process_id, data, sub_path, *args):
		for i, uniprot in enumerate(data):
			...
			wget = f"wget -q {url} -O {save_path}"
			# print(wget, flush=True)
...
class FastaDownloader(Downloader):
    def __init__(self, uniprot_ids, save_dir, **kwargs):
        """
        ...
        save_path = os.path.join(save_dir, "{}.fasta")
        
        super().__init__(data=uniprot_ids, base_url=url, save_path=save_path, **kwargs)
...
def parse_args():
    ...
    parser.add_argument('--type', type=str, required=True, help='Type of download, must be one of [pdb, mmcif, plddt, pae]')
    parser.add_argument('--n_process', type=int, default=1, help='Number of processes to use')
...
if __name__ == '__main__':
    ...
    with open(f'/zhouxibin/workspaces/ESM-Ezy/analysis/IPR_{download_type}_20240526.txt', 'r') as f:
        uniprot_ids = [line.strip() for line in f.readlines()]
    
    # download pdb files
    downloader = FastaDownloader(uniprot_ids, save_dir=f'/zhouxibin/workspaces/ESM-Ezy/data/uniprot_seqs/{download_type}', n_process=n_process, overwrite=True)
    downloader.run()
